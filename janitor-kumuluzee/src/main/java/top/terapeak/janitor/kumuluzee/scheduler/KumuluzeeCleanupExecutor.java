...
import top.terapeak.janitor.executor.CleanupExecutionException;
...
/**
 * Special Kumuluzee executor (due to javax dependencies) that carries out a single cleanup job.
 *
 ...
 */
public class KumuluzeeCleanupExecutor {

    private static final Logger log = LoggerFactory.getLogger(KumuluzeeCleanupExecutor.class);

    // Shared singleton - frameworks may also instantiate their own
    private static final KumuluzeeCleanupExecutor INSTANCE = new KumuluzeeCleanupExecutor();

    public static KumuluzeeCleanupExecutor getInstance() {
        return INSTANCE;
    }
